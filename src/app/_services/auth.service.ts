...
export class AuthService {
    ...
    login(user: User): Observable<any> {
        ...
        return this.http.post('auth/login', body).map(response => response.json()).map(result => {
            if (!result.error) {
    ...
    register(user: User): Observable<any> {
        ...
        return this.http.post('auth/register', body).map(response => response.json()).map(result => {
            if (!result.error) {
                localStorage.setItem('currentUser', JSON.stringify(result));
                this.currentUser.next(result);
                return true;
    ...
    forgotPassword(email: string): Observable<any> {
        return this.http.post('api/forgotpassword', email).map((response) => {
